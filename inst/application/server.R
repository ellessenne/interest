...
function(input, output, session) {
	...
	summ = shiny::reactive({
		...
		if (input$defineMethod != "")
			s$summ[[input$defineMethod]] = factor(s$summ[[input$defineMethod]])
		if (input$defineRefMethod != "")
			s$summ[[input$defineMethod]] = relevel(s$summ[[input$defineMethod]], ref = input$defineRefMethod)
		if (!is.null(input$defineBy))
			for (w in input$defineBy)
				s$summ[[w]] = factor(s$summ[[w]])
		return(s)
	})
					 ...
					 "b_vs_se" =  {
					 	...
					 	if (input$selectOrFacet == "Facet") {
					 		...
					 		if (!is.null(input$plotFacetX) &
					 				!is.null(input$plotFacetY)) {
					 			plot = plot + ggplot2::facet_grid(reformulate(input$plotFacetX, input$plotFacetY),
					 																				labeller = ggplot2::label_both)
					 		} else if (is.null(input$plotFacetX) &
					 							 !is.null(input$plotFacetY)) {
					 			plot = plot + ggplot2::facet_grid(reformulate(".", input$plotFacetY),
					 																				labeller = ggplot2::label_both)
					 		} else if (!is.null(input$plotFacetX) &
					 							 is.null(input$plotFacetY)) {
					 			plot = plot + ggplot2::facet_grid(reformulate(input$plotFacetX, "."),
					 																				labeller = ggplot2::label_both)
					 		}
					 	}
		...
		switch(input$selectPlotSummary,
					 "barplot" = {
					 	...
					 	if (input$plotMCSEConfidenceIntervals)
					 		plot = plot + ggplot2::geom_errorbar(ggplot2::aes_string(ymin = "lower", ymax = "upper"),
					 																				 width = 1 / 3)
					 },
					 "lollyplot" = {
					 	plot = ggplot2::ggplot(df[df$stat == input$selectPlotSummaryStat,],
					 		...
					 		ggplot2::geom_vline(
					 			xintercept = dplyr::case_when(
					 				...
					 				input$selectPlotSummaryStat %in% c("cover", "power") ~ as.double(summ()$level * 100),
					 				input$selectPlotSummaryStat %in% c("bmean", "bmedian") ~ as.double(input$defineTrue)
					 			),
					 			color = "red",
					 			lty = "dotted"
					 		) +
